/*
* mpvuePromise
* https://github.com/youngjuning/mpvuePromise
*
* NPM INSTALL
* https://www.npmjs.com/package/mpvue-promise-pro
*
* regenerator-runtime
* http://t.cn/RuKTciN
...
const mpvuePromise = () => {
  for (let key in mpvue) {
    if (/^on|^create|Sync$|Manager$|^pause/.test(key) && key !== 'createBLEConnection' || key === 'stopRecord' || key === 'stopVoice' || key === 'stopBackgroundAudio' || key === 'stopPullDownRefresh' || key === 'hideKeyboard' || key === 'hideToast' || key === 'hideLoading' || key === 'showNavigationBarLoading' || key === 'hideNavigationBarLoading' || key === 'canIUse' || key === 'navigateBack' || key === 'closeSocket' || key === 'closeSocket' || key === 'pageScrollTo' || key === 'drawCanvas') {
      mpvue.pro[key] = mpvue[key]
    } else {
      mpvue.pro[key] = promisify(mpvue[key])
    }
  }
  ...
  mpvue.pro.match = (type, str) => {
    ...
    // 顶部提示框
    mpvue.pro.showTopTips = (option, that) => {
      return new Promise((resolve, reject) => {
  ...
  // 初始化 echarts
  mpvue.pro.initChart = (option, echarts) => {
    ...
    // echarts 延迟加载模式初始化
    mpvue.pro.lazyInitChart = (option, echarts, componentId, that) => {
     ...
     */
    mpvue.pro.updateChart = (chart, option) => {
  ...
  mpvue.pro.saveImageToPhotosAlbum = (tempFilePath) => {
    return new Promise((resolve, reject) => {
      mpvue.getSetting({
        success: (res) => {
    ...
    // TODO: 增加更多的参数配置
    mpvue.pro.canvasToTempFilePath = (canvasContext) => {
      return new Promise((resolve, reject) => {
